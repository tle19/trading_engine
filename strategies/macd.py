...
class MACDIndicator(Strategy):
    ...
    def generate_signal(self, row):
        ...
        if not self.trade_window((9, 30), (20, 30)) and self.position is None:
            self.ema = None
            self.fast_ema = None
            self.slow_ema = None
            self.signal_ema = None
    ...
    def enter_trade(self, ema, stoch_k, stoch_d, rsi, hist, vol):
        if not (self.stoch_lower < min(stoch_k, stoch_d) and max(stoch_k, stoch_d) < self.stoch_upper):
            return None
        if self.stoch_signal == "long" and rsi > 55 and hist > 0:
            if self.close > ema or vol > 0.025:
                ...
                return signal
        if self.stoch_signal == "short" and rsi < 45 and hist < 0:
            if self.close < ema or vol > 0.025:
                signal = self.sell() 
