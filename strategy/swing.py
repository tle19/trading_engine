...
class SwingMAIndicator:  # SWINGMAINDICATOR REQUIRES STRAT OUTSIDE
    def __init__(self, symbol, x=0.0125, take_profit=0.0075, stop_loss=0.0075, window=10):
        self.symbol = symbol
        self.x = x
    ...
    def _load_history_up_to(self, current_date):
        ...
        self.df_history["date"] = self.df_history["timestamp"].dt.date
    ...
    def update(self, row):
        ...
        if (ts.hour, ts.minute) == (9, 30):
            ...
            prev_days = self.df_history[self.df_history["date"] < ts.date()]
            prev_day = prev_days["date"].max()
            prev_close = prev_days[prev_days["date"] == prev_day].iloc[-1]["close"]
            o = row["open"]
            drop_pct = 1 - (open / prev_close)
            ...
            if self.position is None:
                if drop_pct >= self.x and o < moving_average: 
                    self.position = "long"
        ...
        # --- Exit ---
            ...
            if high >= self.tp_price or low <= self.sl_price:
                self.position = None
